...
# 配置 OpenAI API 金鑰
openai.api_key = "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"
...
def analyze_data():
    try:
        ...
        result = []
        workbook_path = f'{stock}_data.xlsx'
...
def extract_text_from_image():
    try:
        ...
        return jsonify({'error': f'圖片文字提取時發生錯誤：{str(e)}'}), 500
...
def send_to_gpt():
    try:
        ...
        # 調用 extract_text_from_image() 提取圖像中的文字
        response = extract_text_from_image()
        extracted_texts = response.get_json().get('extracted_texts', [])
        combined_texts = '\n'.join(extracted_texts)

        prompt = f"Based on the following image data: {combined_texts}, please provide a detailed stock analysis after I ask next question."
        ...
        return jsonify({'gpt_reply': reply})
    except Exception as e:
        return jsonify({'error': f'GPT 回應失敗：{str(e)}'}), 500
